...
- Front = `public/index.html`, **VERSION courante = v208** (en prod). Monter `const VERSION='vNNN'` à chaque livraison front.
...
1bis. **Liste « lookalike » depuis une fiche** (demande Didier 21/07) : bouton « Créer une liste lookalike » sur une fiche → ouvre la Liste intelligente avec le prompt prérempli à partir des données structurées réelles de la fiche (NAF exact, effectif, zone, activité GMB) + personas au choix du SDR. Le prompt libre « … dans des boîtes comme X » fonctionne déjà (règle codée dans `api/ia-liste.js`), le bouton ne fait que fiabiliser/préremplir. Dev faible : préremplissage front + 2-3 lignes de contexte dans le prompt.
